...
Railway Limited Trial показывает одноразовый кредит $5 и 30 дней; платёжный метод не добавлен. После исчерпания кредита Railway остановит deployments. После короткого probe интерфейс всё ещё показывал доступные $5 и округлял текущий расход до $0.00; этого недостаточно, чтобы гарантировать доступность до 30 сентября 2026 года. Проверяйте `Workspace → Usage` и публичный healthcheck ежедневно, особенно перед прогоном организаторов. Публичный `/metrics` доступен без авторизации, как и `/process`; не добавляйте в запросы реальные ПД. [Короткий probe на Railway](../submission/railway-load-probe-2026-09-23.md) дал 12 транспортных сбоев k6 из 3001 при 100 RPS. Railway в том же окне учёл около 3,2 тыс. запросов как 2xx, показал 0% HTTP errors и не показал насыщения приложения или Redis, поэтому серверная перегрузка не подтверждается; точный источник до Railway ingress не локализован. Полный профиль 1000 RPS / 300 секунд на Railway не проводился. Локальный benchmark не доказывает производительность пробного хостинга.
