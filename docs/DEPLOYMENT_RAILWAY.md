...
Railway Limited Trial показывает одноразовый кредит $5 и 30 дней; платёжный метод не добавлен. После исчерпания кредита Railway остановит deployments. После нагрузочных прогонов интерфейс всё ещё показывал доступные $5 и округлял текущий расход до $0.00; этого недостаточно, чтобы гарантировать доступность до 30 сентября 2026 года. Проверяйте `Workspace → Usage` и публичный healthcheck ежедневно, особенно перед прогоном организаторов. Публичный `/metrics` доступен без авторизации, как и `/process`; не добавляйте в запросы реальные ПД. [Отчёт Railway](../submission/railway-load-probe-2026-09-23.md) фиксирует успешный probe 100 RPS / 30 секунд и провал полного профиля 1000 RPS / 300 секунд: около 33% ошибок, 76483 ответа 429, 10019 dropped iterations в baseline и p95 около 10 секунд. Этот deployment подходит для функциональной демонстрации, но не подтверждает целевую производительность §7.1. Локальный benchmark остаётся отдельным доказательством и не переносится на пробный хостинг.
