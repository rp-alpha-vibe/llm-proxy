...
Подробный сквозной алгоритм поиска, маскирования, временного хранения и восстановления данных с примерами описан в [MASKING.md](MASKING.md). Ниже остаются технические решения, интерфейсы и границы компонентов.
...
Колонка ниже — где доказательство должно появиться, а не список уже полученных результатов. Для E0–E13 и E15.1–E15.10 есть контракт `/process`, повторные запросы, точное восстановление и восстановление в новом ответе модели, изоляция настроек, контролируемый отказ при перегрузке, шифрование временных записей, локальные проверки качества и безопасности, журналы и технические показатели, локальный нагрузочный прогон и исходный архив с проверкой запуска. Публичный HTTPS-адрес развёрнут на Railway: https://llm-proxy-production-84c7.up.railway.app (порядок развёртывания — в `docs/DEPLOYMENT_RAILWAY.md`). Проверка E14 на официальной системе, окончательная отправка E15.11 и подтверждение устойчивости Railway при целевой нагрузке ещё открыты. Локальный результат по нагрузке не следует приписывать публичному размещению.
