...
- для ответа LLM: восстанавливаются известные маски внутри нового текста ответа, в том числе при перестановке/повторении; неизвестные/неоднозначные placeholders остаются без изменения, а сохранённый prompt не подменяет ответ (§5.1 requirements);
