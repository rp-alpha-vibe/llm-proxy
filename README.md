...
E0–E13 и E15.1–E15.10 завершены в части правил, renderer-ов, product demask, локального quality gate, security tests, observability, локального load baseline и упаковки исходного ZIP. На этой машине (24 CPU, Redis 7, 8 workers) окно 300 с после 15 с прогрева дало 300001 успешную baseline-операцию, 0 ошибок и 0 ответов 429. p50 3.78 мс, p95 21.78 мс, p99 57.15 мс, max 286.37 мс; offered RPS 1000; attempts равны успехам. Working set Python-процессов около 585–701 МБ; суммарный CPU process time вырос примерно на 772 с за ~302 с окна. Lifetime Redis latency после прогонов около 18 мс среднее; чистое окно отдельно не изолировалось. Это не официальный прогон организаторов и не 2000 RPS. На каждый `POST /process` пишется один JSON completion log без сырых ПД: operation, status, latency, типы и число ПД, а также `detect_ms` / `redis_ms` / `total_ms`. Security tests проверяют, что синтетические ФИО, телефон, email, PAN и паспорт не попадают в логи, ответ `/metrics`, plaintext Redis и тела ошибок. Сессии разных потребителей и `payload_id` не раскрывают mapping. Ключ шифрования задаётся через окружение, `.env` не коммитится. CI поднимает Redis 7 и требует HTTP/Redis smoke и TTL без тихого skip. `POST /process` принимает до 8 000 000 символов и проходит policy и `ProcessService` до Redis: новый payload маскируется, повтор исходного текста возвращает ту же маску, точная маска восстанавливает исходный текст; профиль 100 000 токенов покрыт HTTP-тестом с Redis. Синтетический ответ LLM восстанавливает известные однозначные маски `placeholder` и `competition` при перестановке и повторе, сохраняет новый текст и оставляет неизвестные или неоднозначные токены. Policy `placeholder` маскирует как `[[PII:TYPE:n]]`, policy `competition` — как `<TYPE_n>`. Второй формат — локальное обратимое допущение, пока нет официального образца маски. `python -m llm_proxy.quality` на 46 синтетических fixtures дал локальный F1 1.0 по каждой обязательной категории, round-trip 1.0 и ноль срабатываний на negative fixtures. Пороги в `gate.py` не снижались. Это не официальный span-based scoring. Локальный NLP/NER не добавлялся: E7.12–E7.13 остаются открытыми, потому что этот корпус не показал quality gap. Async-тесты запускаются через `pytest-asyncio`. `scripts/package.py` собирает source ZIP с отдельной инструкцией запуска, harden allowlist и Docker smoke mask/retry/unmask; job `package-smoke` в CI это повторяет. E15.11 открыт: нужны презентация, ссылка на VCS и публичный URL. Официальный способ допуска AlfaSonar (E14) остаётся открытой границей и блокером submission readiness.
